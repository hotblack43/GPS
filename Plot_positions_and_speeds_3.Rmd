...
get_jump_speeds_by_postion_modelling <- function(df,name,limitdates)
{
  ...
  nlimits <- length(limitdates)
  statname <- strsplit(strsplit(name, "/")[[1]][2],".rds")[[1]][1]
  pred_at_interval_left_right <- NULL
  ...
  for (ilimit in seq(from=1,to=(nlimits-1),by=2))
  {
    ...
    if (ilimit == 1) {
      ...
      pred_at_interval_left_right  <- c(first(rlmfit$fitted.values), last(rlmfit$fitted.values))
    }
    if (ilimit  > 1) {
      points(df$POSIX[idx],df$Latitude[idx])
      lines(df$POSIX[idx],rlmfit$fitted.values,col=2,lwd=3)
      #
      pred_at_interval_left_right  <- rbind.data.frame(pred_at_interval_left_right, c(first(rlmfit$fitted.values), last(rlmfit$fitted.values)))
      
    }
    
  }
  # cal cthe actuall offsets
  listen <- NULL
  for (i in 2:nrow(pred_at_interval_left_right))
  {
    listen <- rbind(listen,c(i,pred_at_interval_left_right[i,2]-pred_at_interval_left_right[i-1,1]))
  }
  browser()
  return(pred_at_interval_left_right)
}
```
