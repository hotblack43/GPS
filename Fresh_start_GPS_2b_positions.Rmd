...
model_motion2 <- function(df, name, limitdates)
{
  ...
  for (ilimit in 1:nlimits)
  {
    idx <-
      which(df$POSIX >= limitdates[ilimit, 1] &
              df$POSIX < limitdates[ilimit, 2] & !is.na(df$Latitude))
    if (length(idx) != 0) {
      ...
      if (ilimit == 1) {
        ...
        points(df$POSIX[idx], df$Latitude[idx], pch=19, cex=0.2)
        lines(df$POSIX[idx],
  ...
  for (ilimit in 1:nlimits)
  {
    idx <-
      which(df$POSIX >= limitdates[ilimit, 1] &
              df$POSIX < limitdates[ilimit, 2] & !is.na(df$Longitude))
    if (length(idx) != 0) {
      ...
      if (ilimit == 1) {
        ...
        points(df$POSIX[idx], df$Longitude[idx], pch=19, cex=0.2)
        lines(df$POSIX[idx],
              rlmfit$fitted.values,
              col = 2,
              lwd = 3)
  ...
  
{
  ...
  print(surge_speeds)
  #browser()
  print(length(segspeeds))
  print(dim(surge_speeds$speed_jump))
  # 
  for (ijk in 1:(length(segspeeds)-1))
  {
    print(c(ijk,segspeeds[ijk]))
    print(c(ijk,surge_speeds$speed_jump[ijk,2]))
  }
  print(c(ijk,segspeeds[length(segspeeds)]))
}
print("------------------------------------------")
```
