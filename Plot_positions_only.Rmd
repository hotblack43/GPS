...
# Plot one map with all movements
```{r}
idx <- which(data$lon > -69)   # -169 try also!
minlon <- min(data$lon[idx])
maxlon <- max(data$lon[idx])
minlat <- min(data$lat[idx])
maxlat <- max(data$lat[idx])
#
colids <- c("red","green","blue","grey","orange","yellow","black")
plot(data$lon[idx],data$lat[idx],xlab='longitude',ylab='latitude',pch=19,cex=0.1)
for (i in 1:length(IDs))
{
  idx <- which(data$UnitID == IDs[i])
  print(colids[i])
  points(data$lon[idx],data$lat[idx],col=colids[i],pch=19,cex=0.1,type="b")
