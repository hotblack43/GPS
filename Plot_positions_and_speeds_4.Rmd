...
model_motion2 <- function(df,name,limitdates)
{
  ...
  {
    idx <- which(df$POSIX >= limitdates[ilimit] & df$POSIX < limitdates[ilimit+1] & !is.na(df$lat) )
  ...
  for (ilimit in seq(from=1,to=(nlimits-1),by=2))
  {
    idx <- which(df$POSIX >= limitdates[ilimit] & df$POSIX < limitdates[ilimit+1] & !is.na(df$lon) )
