...
DOES NOT WORK YET model_speed comes up with idx empty
...
model_motion <- function(df,statname,limitdates)
{
  ...
  for (isegment in 1:(nsegments-1))
  {
    ...
    # second segment
    idx <- which(df$UTC >= limitdates[isegment+1,1] & df$UTC < limitdates[isegment+1,2] )
        print(c(anytime(limitdates[isegment,1]),anytime(limitdates[isegment,2])))
    print(length(idx))
    rlmfit2 <- rlm(df$lat[idx] ~ df$UTC[idx])
    lat_speed2 <- summary(rlmfit2)$coefficients[2]*3600/180*pi # radians per hour
    # speed during jump between segments
    lat_jump <- first(rlmfit2$fitted.values)-last(rlmfit1$fitted.values)
    lat_speed <- rbind(lat_speed,c(isegment,lat_speed1,lat_jump))
    # lon
    # first segment
    idx <- which(df$UTC >= limitdates[isegment,1] & df$UTC < limitdates[isegment,2] )
    rlmfit1 <- rlm(df$lon[idx] ~ df$UTC[idx])
    lon_speed1 <- summary(rlmfit1)$coefficients[2]*3600/180*pi # radians per hour
    # second segment
    idx <- which(df$UTC >= limitdates[isegment+1,1] & df$UTC < limitdates[isegment+1,2] )
    rlmfit2 <- rlm(df$lon[idx] ~ df$UTC[idx])
    lon_speed2 <- summary(rlmfit2)$coefficients[2]*3600/180*pi # radians per hour
    # speed during jump between segments
    lon_jump <- first(rlmfit2$fitted.values)-last(rlmfit1$fitted.values)
  ...
  colnames(lat_speed) <- c("segment_number","lat1_speed_radperhr","lat_jump_between_radians")
  colnames(lon_speed) <- c("segment_number","lon1_speed_radperhr","lon_jump_between_radians")
...
get_jump_speeds <- function(listerne,lon_in,lat_in)
{
  ...
  {
    delta_lon <- listerne$longs[iseg+1,1]-listerne$longs[iseg,2] # in degrees
    delta_lon <- delta_lon/180*pi # in radians
    delta_lat  <- listerne$lats[iseg+1,1]-listerne$lats[iseg,2] # in degrees
    delta_lat <- delta_lat/180*pi # in radians
...
important_times <- c(as.POSIXct("2022-03-31 00:00:00",tz="UTC"),as.POSIXct("2022-04-24 00:00:00",tz="UTC"),
                     as.POSIXct("2022-04-24 00:00:00",tz="UTC"),as.POSIXct("2022-04-27 17:00:00",tz="UTC"),
                     as.POSIXct("2022-04-27 17:00:00",tz="UTC"),as.POSIXct("2022-05-31 00:00:00",tz="UTC"),
                     as.POSIXct("2022-05-31 00:00:00",tz="UTC"),as.POSIXct("2022-06-09 03:00:00",tz="UTC"),
                     as.POSIXct("2022-06-09 03:00:00",tz="UTC"),as.POSIXct("2022-07-09 12:00:00",tz="UTC")
)
limitdates <- NULL
for (it in seq(from=1,to=length(important_times),by=2))
{  limitdates <- rbind(limitdates,c(important_times[it],important_times[it+1]))}

for (i_unitID in unitIDs)
{
  print("------------------------------------------")
  print(paste(" Processing unitID ",i_unitID))
  idx <- which(data$UnitID == i_unitID)
  df <- data[idx,]
  ...
  speeds <- get_jump_speeds(listerne,lon=median(df$lon,na.rm=T),lat=median(df$lat,na.rm=T))
  print(speeds)
