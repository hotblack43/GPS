...
model_motion2 <- function(df,name,limitdates)
{
  ...
  plot(df$POSIX,df$Latitude,type="p",xlim=range(df$POSIX),ylim=range(df$Latitude,na.rm=T),xlab="Time",ylab="Latitude",main=namesnumbers[which(namesnumbers[,2] == statname),1])
  ...
  plot(df$POSIX,df$Longitude,type="p",xlim=range(df$POSIX),ylim=range(df$Longitude,na.rm=T),xlab="Time",ylab="Longitude",main=namesnumbers[which(namesnumbers[,2] == statname),1])
  for (ilimit in 1:nlimits)
  {
